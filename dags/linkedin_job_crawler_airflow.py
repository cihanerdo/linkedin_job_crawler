...
def upload_to_gcs(data_folder, **kwargs):
    ...
    # Upload files from job_ids folder
    upload_files_to_gcs(job_ids_folder, 'Job_Ids', bucket_name, gcs_conn_id, kwargs)
...
def upload_files_to_gcs(local_folder, gcs_folder, bucket_name, gcs_conn_id, kwargs):
    ...
    for csv_file in csv_files:
        ...
        upload_task = LocalFilesystemToGCSOperator(
            task_id=f'upload_to_gcs_{sanitized_csv_file}',
            src=local_file_path,
            dst=gcs_file_path,
            bucket=bucket_name,
...
def check_csv_files_existence(data_folder, **kwargs):
    try:
        ...
        if job_ids_files or job_details_files:
            return ['upload_to_gcs_task']
        else:
            return ['no_op']
    except Exception as e:
